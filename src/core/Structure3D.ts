  ...
  update() {
    ...
    for (let i = 0; i < this.numChains; i++) {
      ...
      if (hostChainNumber !== -1) {
        ...
        // Now that we've clamped the base location of this chain to the start or end point of the bone in the chain we
        // are connected to, it's time to deal with any base bone constraints...
        ...
        switch (constraintType) {
          ...
          case GLOBAL_ROTOR: // Nothing to do because the basebone constraint is not relative to bones in other chains
                             // in this structure
          case GLOBAL_HINGE: // Nothing to do because the basebone constraint is not relative to bones in other chains in this
                   // structure
            break;

          // If we have a local rotor or hinge constraint then we must calculate the relative basebone constraint
          // before calling solveForTarget
          case LOCAL_ROTOR:
          case LOCAL_HINGE:
  ...
  addBoneMesh(bone, prev, ar, chain) {
    ...
    switch (type) {
      ...
      case J_GLOBAL :
        ...
        extraGeo.rotateX(-Math.PI * 0.5);
        //extraGeo.applyMatrix4( new this.THREE.Matrix4().makeRotationX( -Math.PI*0.5 ) );
        if (axe.z === 1) extraGeo.rotateX(-Math.PI * 0.5);//extraGeo.applyMatrix4( new
                                                          // this.THREE.Matrix4().makeRotationX( -Math.PI*0.5 ) );
        if (axe.y === 1) {
          extraGeo.rotateY(-Math.PI * 0.5);
          extraGeo.rotateX(-Math.PI * 0.5)
        }//{extraGeo.applyMatrix4( new this.THREE.Matrix4().makeRotationY( -Math.PI*0.5 ) );extraGeo.applyMatrix4( new
         // this.THREE.Matrix4().makeRotationX( -Math.PI*0.5 ) );}
        if (axe.x === 1) {
