import { END, GLOBAL_HINGE, GLOBAL_ROTOR, J_BALL, J_GLOBAL, J_LOCAL, LOCAL_HINGE, LOCAL_ROTOR, MAX_VALUE, NONE } from "../constants.js";
...
var Chain3D = /** @class */ (function () {
    ...
    Chain3D.prototype.setFixedBaseMode = function (value) {
        // Enforce that a chain connected to another chain stays in fixed base mode (i.e. it moves with the chain it's
        // connected to instead of independently)
        if (!value && this.connectedChainNumber !== -1)
            return;
        if (this.baseboneConstraintType === GLOBAL_ROTOR && !value)
            return;
    ...
    Chain3D.prototype.solveForTarget = function (t) {
        ...
        while (i--) {
            ...
            if (solveDistance < bestSolveDistance) {
                ...
                if (Math.abs(solveDistance - lastPassSolveDistance) < this.minIterationChange)
                    break;
            }
            ...
            if (i !== this.numBones - 1) {
                ...
                switch (jointType) {
                    ...
                    case J_GLOBAL:
                        ...
                        // Project this bone outer-to-inner direction onto the hinge rotation axis
                        boneOuterToInnerUV.projectOnPlane(hingeRotationAxis);
                        // NOTE: Constraining about the hinge reference axis on this forward pass leads to poor solutions... so we
                        // won't.
                        break;
                    case J_LOCAL:
                        if (i > 0) { // Not a basebone? Then construct a rotation matrix based on the previous bones inner-to-to-inner direction...
                ...
                switch (jointType) {
                    ...
                    case J_GLOBAL:
                        hingeRotationAxis = joint.getHingeRotationAxis();
                        // Global hinges get constrained to the hinge rotation axis, but not the reference axis within the hinge
                        // plane
                        boneOuterToInnerUV.projectOnPlane(hingeRotationAxis);
                        break;
                    case J_LOCAL:
                        // Local hinges get constrained to the hinge rotation axis, but not the reference axis within the hinge
                        // plane
                ...
                bone.setStartLocation(newStartLocation);
                // ...and set the end joint location of the bone further in to also be at the new start location (if there IS a
                // bone further in - this may be a single bone chain)
                if (i > 0)
                    this.bones[i - 1].setEndLocation(newStartLocation);
            }
        } // End of forward pass
            ...
            if (i !== 0) {
                ...
                switch (jointType) {
                    ...
                    case J_GLOBAL:
                        ...
                        if (!joint.freeHinge)
                            boneInnerToOuterUV.constrainedUV(hingeReferenceAxis, hingeRotationAxis, tmpMtx, joint.min, joint.max);
                        break;
                    case J_LOCAL:
                ...
                var boneInnerToOuterUV = bone.getDirectionUV();
                var hingeRotationAxis_1 = void 0, hingeReferenceAxis_1 = void 0;
                switch (this.baseboneConstraintType) {
                    ...
                    case GLOBAL_HINGE:
                        ...
                        hingeReferenceAxis_1 = joint.getHingeReferenceAxis();
                        ...
                        if (!joint.freeHinge)
                            boneInnerToOuterUV.constrainedUV(hingeReferenceAxis_1, hingeRotationAxis_1, tmpMtx, joint.min, joint.max);
                        break;
                    case LOCAL_HINGE:
                        hingeRotationAxis_1 = this.baseboneRelativeConstraintUV;
                        hingeReferenceAxis_1 = this.baseboneRelativeReferenceConstraintUV;
                        // Get the inner-to-outer direction of this bone and project it onto the global hinge rotation axis
