import { END, GLOBAL_ABSOLUTE, J_LOCAL, LOCAL_ABSOLUTE, MAX_VALUE, NONE } from "../constants.js";
...
var Chain2D = /** @class */ (function () {
    ...
    Chain2D.prototype.solveForTarget = function (t) {
        ...
        // Update our last base and target locations so we know whether we need to solve for this start/end configuration
        // next time
    ...
    Chain2D.prototype.solveIK = function (target) {
        ...
        for (i = 0; i < this.numBones; i++) {
            ...
            if (i !== 0) {
                ...
                if (this.baseboneConstraintType === NONE) {
                    // Calculate the new end location as the start location plus the direction multiplyScalar by the length of
                    // the bone
                    endPosition = bone.start.plus(directionUV.multiplyScalar(boneLength));
                    ...
                    // LOCAL_ABSOLUTE? (i.e. local-space directional constraint) - then we must constraint about the relative
                    // basebone constraint UV...
